# -*- coding: utf-8 -*-
import os, glob, contextlib, io
import torch, numpy as np, lightning as pl
...
def quiet_stdout(enabled=True):
    if not enabled:
        yield
        return
    buf, old = io.StringIO(), os.dup(1)
    try:
        with contextlib.redirect_stdout(buf):
            yield
    finally:
        pass  # we don't need the captured text; just suppress


# ---------- robust tensor → Pose (torso anchor + optional axis swap) ----------
def tensor_to_pose(t_btjc, header, swap_yz=True, invert_z=True):
    """
    t_btjc: [T,J,C] or [1,T,J,C], C=3 in (x,y,z).
    Rendering-only normalization:
      - center by torso (shoulders + hips median)
      - scale by 95th percentile radius from torso
      - optional swap/invert for y/z to fix 'lying/rolling' look
    """
    t = t_btjc[0] if t_btjc.dim() == 4 else t_btjc
    arr = t.detach().cpu().numpy().astype(np.float32)  # [T,J,3]

    # ---- axis rectify (visualization only) ----
    if swap_yz:
        arr = arr[..., [0, 2, 1]]  # (x, z, y)
    if invert_z and arr.shape[-1] >= 3:
        arr[..., 2] *= -1.0

    # ---- torso indices (MediaPipe: 11,12 shoulders; 23,24 hips) ----
    torso_idx = [11, 12, 23, 24]
    torso_idx = [i for i in torso_idx if i < arr.shape[1]]
    if len(torso_idx) >= 2:
        torso_xy = arr[:, torso_idx, :2].reshape(-1, 2)
        ctr_xy = np.median(torso_xy, axis=0, keepdims=True)  # [1,2]
    else:
        ctr_xy = np.median(arr[:, :, :2].reshape(-1, 2), axis=0, keepdims=True)

    arr[:, :, :2] -= ctr_xy  # center by torso
...
def autoregressive_generate(model: LitMinimal, past_btjc: torch.Tensor, sign_img: torch.Tensor, future_len: int):
    # 显式设置 chunk=1；尽量覆盖模型内部的默认长度限制
    if hasattr(model, "future_max_len"):
        try:
            setattr(model, "future_max_len", int(future_len))
        except Exception:
            pass
    return model.sample_autoregressive_diffusion(
        past_btjc=past_btjc, sign_img=sign_img, future_len=int(future_len), chunk=1
    )
...
if __name__ == "__main__":
    ...
    loader = DataLoader(
        small_ds, batch_size=2, shuffle=True,
        collate_fn=zero_pad_collator, num_workers=0, pin_memory=False
    )
    ...
    model = LitMinimal(
        num_keypoints=J, num_dims=C, lr=3e-4,
        stats_path=os.path.join(data_dir, "mean_std_178.pt"),
        diffusion_steps=200, beta_start=1e-4, beta_end=2e-2,
        pred_target="x0", guidance_scale=0.0,
    ).eval()

    probe = sanitize_btjc(batch0["data"][:1])
    std_probe = model.normalize(probe).float().std().item()
    status = "OK" if 0.5 <= std_probe <= 2.0 else "MISMATCH"
    print(f"[CHECK] GT_norm.std = {std_probe:.2f} → {status}")
    if status == "MISMATCH":
        with torch.no_grad():
            model.std_pose *= max(std_probe, 1e-3)
        std_after = model.normalize(probe).float().std().item()
        print(f"[Calib] scaled std_pose by {std_probe:.3f} → recheck std={std_after:.2f}")

    for obj in (model, getattr(model, "model", None)):
        if obj is not None and hasattr(obj, "verbose"):
            try: obj.verbose = False
            except: pass
    ...
    # ---- save GT (raw / unnorm) with torso anchoring vis ----
    ...
    # ---- Autoregressive generation (T_future frames) ----
    print("=== Inference B (autoregressive, multi-frames) ===")
    with quiet_stdout(enabled=True):  # 尽量压住内部 debug 噪声
        gen_un = autoregressive_generate(model, past_raw, sign_img, future_len=T_future)
    ...
    if gen_un.size(1) > 1:
        ...
        print(f"[GEN MOTION] SKIPPED (T={gen_un.size(1)})")
    print(f"[DEBUG] gen_un shape = {tuple(gen_un.shape)} (should be [1,{T_future},178,3])")
    ...
    pred_motion = motion_stats(pred_un)     # 单帧 → 0 是预期
    ...
    l2_full, l2_early, l2_late = l2_error(pred_un, fut_raw[:, :1])
    ...
    def axis_stats(x):
        if x.dim() == 4: x = x[0]
        m = x.mean(dim=(0, 1)); s = x.std(dim=(0, 1))
