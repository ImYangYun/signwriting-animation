...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        past_btjc = cond["input_pose"][:1].to(model.device)     # [1,Tp,J,C]
        sign_img  = cond["sign_image"][:1].to(model.device)     # [1,3,224,224]
        fut_gt    = batch["data"][:1].to(model.device)          # [1,Tf,J,C]
        mask_bt   = cond["target_mask"][:1].to(model.device)    # [1,Tf] 或更高维

        print("[GEN] using generate_full_sequence", flush=True)
        gen_btjc = model.generate_full_sequence(
            past_btjc=past_btjc,
            sign_img=sign_img,
            target_mask=mask_bt,       # 用 mask 推断 Tf（动态长度）
            # 如需固定长度：target_len=int(mask_bt.sum(dim=1).item())
        )  # [1,Tf,J,C]
