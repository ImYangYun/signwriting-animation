...
Speed: training <10min, sampling <5s（tiny subset 设置）.
"""
...
class FastFuturePredictor(pl.LightningModule):
    """
    使用 CAMDM 的 backbone，但不走 diffusion；
    直接预测整段 future 序列 → 非常快。
    Loss = MSE + 0.3 * velocity L1.
    """
    ...
    def __init__(self,
                 ...
                 num_dims: int = 3,
                 lr: float = 1e-4,
                 stats_path: str = "/data/yayun/pose_data/mean_std_178.pt"):
        super().__init__()
        self.save_hyperparameters()

        # ---------- mean / std ----------
        stats = torch.load(stats_path, map_location="cpu")
        # stats["mean"].shape = [178*3] = [534]
        mean = stats["mean"].float().view(1, 1, -1, 3)   # [1,1,J,3]
        std  = stats["std"].float().view(1, 1, -1, 3)
        self.register_buffer("mean_pose", mean)
        self.register_buffer("std_pose", std)

        # ---------- backbone（不用 diffusion，只借它的 encoder） ----------
    ...
    # ---------------------- forward ----------------------
    def forward(self, past_btjc: torch.Tensor, future_len: int, sign_img: torch.Tensor):
        """
        past_btjc: [B, Tp, J, C]
        sign_img : [B, 3, H, W]（你原来的 shape）
        return   : [B, Tf, J, C]
        """
        B, Tp, J, C = past_btjc.shape

        # encoder_only 的具体实现你仓库里已经有了，这里假设返回 [B, J*C]
        enc = self.backbone.encoder_only(
            self.normalize(past_btjc),  # [B,Tp,J,C]
            sign_img                   # sign image 条件
        )                              # -> [B, J*C]（如果是别的 shape，你之前那版也会一起报错）

        # MLP 映射后 reshape 回一帧的 [B,1,J,C]，再 repeat 到 Tf 帧
        x = self.mlp(enc)             # [B, J*C]
        x = x.view(B, 1, J, C)        # 一帧
        x = x.repeat(1, future_len, 1, 1)  # [B,Tf,J,C]
    ...
    def training_step(self, batch, _):
        ...
        fut  = sanitize_btjc(batch["data"]).float()             # [B,Tf,J,C]
        past = sanitize_btjc(cond["input_pose"]).float()        # [B,Tp,J,C]
...
# Helper functions
# ============================================================
def _plain(x: torch.Tensor) -> torch.Tensor:
    if hasattr(x, "tensor"):
        x = x.tensor
...
def temporal_smooth(x: torch.Tensor, k: int = 5) -> torch.Tensor:
...
def recenter_pair(gt: torch.Tensor, pr: torch.Tensor):
    if gt.dim() == 4:
        gt = gt[0]
    if pr.dim() == 4:
        pr = pr[0]
    ...
    pts = torch.cat(
        [gt[..., :2].reshape(-1, 2), pr[..., :2].reshape(-1, 2)],
        dim=0,
    )
...
if __name__ == "__main__":
    ...
    # -------------------------------
    # Dataset (tiny subset for speed)
    # -------------------------------
    def make_loader(split: str, subset: int | None = 16):
