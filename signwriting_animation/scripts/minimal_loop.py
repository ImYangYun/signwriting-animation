# -*- coding: utf-8 -*-
"""
End-to-end 178-joint pipeline:
- Training (Lightning Trainer)
- Validation
- Sampling future sequence
- Visualization stable
- Save .pose for GT and PRED
"""

import os
import sys
import torch
import numpy as np
import lightning as pl
import matplotlib.pyplot as plt
plt.switch_backend("Agg")

from torch.utils.data import DataLoader
from pose_format import Pose
from pose_format.numpy.pose_body import NumPyPoseBody
from pose_format.torch.masked.collator import zero_pad_collator
from pose_format.utils.generic import reduce_holistic

from signwriting_animation.data.data_loader import DynamicPosePredictionDataset
from signwriting_animation.diffusion.lightning_module import LitMinimal, sanitize_btjc


# ---------------------------------------------------------
# Visualization helpers
# ---------------------------------------------------------

def _plain(x):
    if hasattr(x, "tensor"): x = x.tensor
    if hasattr(x, "zero_filled"): x = x.zero_filled()
    return x.detach().cpu().contiguous().float()

def temporal_smooth(x, k=5):
    import torch.nn.functional as F
    if x.dim() == 4:
        x = x[0]
    T,J,C = x.shape
    x = x.permute(2,1,0).reshape(1, C*J, T)
    x = F.avg_pool1d(x, k, stride=1, padding=k//2)
    x = x.reshape(C, J, T).permute(2,1,0).contiguous()
    return x.unsqueeze(0)

def recenter_for_view(x):
    if x.dim() == 4: x = x[0]
    x = torch.nan_to_num(x, nan=0.0)

    torso = x[:, :33, :2].reshape(-1, 2)
    center = torso.median(dim=0).values
    x[..., :2] -= center

    pts = x[..., :2].reshape(-1, 2)
    q02 = torch.quantile(pts, 0.02, dim=0)
    q98 = torch.quantile(pts, 0.98, dim=0)
    span = (q98 - q02).clamp(min=50.0)

    scale = min(450.0/span.max(), 5.0)
    x[..., :2] *= scale

    x[..., 0] += 256
    x[..., 1] += 256
    return x.unsqueeze(0)

def tensor_to_pose(x_btjc, header):
    if x_btjc.dim() == 4:
        x_btjc = x_btjc[0]
    arr = np.ascontiguousarray(x_btjc[:,None,:,:], dtype=np.float32)
    conf = np.ones((arr.shape[0],1,arr.shape[2],1), dtype=np.float32)
    return Pose(header=header, body=NumPyPoseBody(fps=25, data=arr, confidence=conf))


# ---------------------------------------------------------
# MAIN: Training + Eval + Sampling + Visualization
# ---------------------------------------------------------

if __name__ == "__main__":
    pl.seed_everything(42)

    # -----------------------------------------
    # Paths & Config
    # -----------------------------------------
    data_dir = "/data/yayun/pose_data"
    csv_path = "/data/yayun/signwriting-animation/data_fixed.csv"
    mean_std_178 = os.path.join(data_dir, "mean_std_178.pt")

    out_dir = "logs/minimal_178_all"
    os.makedirs(out_dir, exist_ok=True)

    BATCH_SIZE   = 8
    MAX_EPOCHS   = 50
    NUM_WORKERS  = 4

    # -----------------------------------------
    # Dataset: Already 178 joints
    # -----------------------------------------
    def make_loader(split):
        ds = DynamicPosePredictionDataset(
            data_dir=data_dir,
            csv_path=csv_path,
            num_past_frames=60,
            num_future_frames=30,
            with_metadata=True,
            split=split,
            reduce_holistic=True,    # 💙 关键：直接输出 178 joints
        )
        return DataLoader(
            ds,
            batch_size=BATCH_SIZE,
            shuffle=(split=="train"),
            num_workers=NUM_WORKERS,
            collate_fn=zero_pad_collator,
            pin_memory=True,
            persistent_workers=True if NUM_WORKERS > 0 else False
        )

    train_loader = make_loader("train")
    val_loader   = make_loader("dev")

    print("[INFO] Train samples:", len(train_loader.dataset))
    print("[INFO] Val samples:",   len(val_loader.dataset))

    # -----------------------------------------
    # Model (178 joints)
    # -----------------------------------------
    model = LitMinimal(
        num_keypoints=178,          # 💙 从此统一 178 joints
        num_dims=3,
        lr=1e-4,
        stats_path=mean_std_178,    # 💙 使用 mean_std_178
        diffusion_steps=1000,
        pred_target="x0",
        guidance_scale=0.0,
    )

    # -----------------------------------------
    # Trainer
    # -----------------------------------------
    trainer = pl.Trainer(
        default_root_dir=out_dir,
        accelerator="gpu",
        devices=1,
        max_epochs=MAX_EPOCHS,
        log_every_n_steps=10,
        enable_checkpointing=True,
    )

    # -----------------------------------------
    # TRAIN
    # -----------------------------------------
    print("===== TRAINING 178-joint diffusion =====")
    trainer.fit(model, train_loader, val_loader)
    print("===== TRAIN DONE =====")

    # -----------------------------------------
    # SAMPLE (Inference)
    # -----------------------------------------
    print("\n===== SAMPLING FUTURE POSE =====")
    model.eval()

    batch = next(iter(val_loader))
    cond = batch["conditions"]

    past_raw = sanitize_btjc(cond["input_pose"][:1]).to(model.device)
    fut_raw  = sanitize_btjc(batch["data"][:1]).to(model.device)
    sign_img = cond["sign_image"][:1].to(model.device)

    pred_178 = model.sample_autoregressive_diffusion(
        past_btjc=past_raw,
        sign_img=sign_img,
        future_len=fut_raw.size(1),
        chunk=1
    )

    # -----------------------------------------
    # UNNORMALIZE
    # -----------------------------------------
    ms = torch.load(mean_std_178)
    mean = torch.tensor(ms["mean"], device=model.device)
    std  = torch.tensor(ms["std"], device=model.device)

    fut_un  = fut_raw  * std + mean
    pred_un = pred_178 * std + mean

    fut_un  = _plain(fut_un)
    pred_un = _plain(pred_un)

    # -----------------------------------------
    # Visualization Stabilization
    # -----------------------------------------
    pred_un = torch.clamp(pred_un, -2000, 2000)

    fut_s   = temporal_smooth(fut_un)
    pred_s  = temporal_smooth(pred_un)

    fut_vis  = recenter_for_view(fut_s)
    pred_vis = recenter_for_view(pred_s)

    # -----------------------------------------
    # Build 178 header
    # -----------------------------------------
    pose_path = batch["records"][0]["pose"]
    with open(os.path.join(data_dir, pose_path), "rb") as f:
        pose = Pose.read(f)
    header = reduce_holistic(pose.remove_components(["POSE_WORLD_LANDMARKS"])).header

    # -----------------------------------------
    # Save .pose files
    # -----------------------------------------
    out_gt   = os.path.join(out_dir, "gt_178.pose")
    out_pred = os.path.join(out_dir, "pred_178.pose")

    tensor_to_pose(fut_vis, header).write(open(out_gt, "wb"))
    tensor_to_pose(pred_vis, header).write(open(out_pred, "wb"))

    print("[SAVE] GT saved:", out_gt)
    print("[SAVE] PRED saved:", out_pred)
    print("\n===== DONE =====")
