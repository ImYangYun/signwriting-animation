...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        Tf = gen_btjc.size(1)
        print(f"[GEN] predicted length Tf={Tf}")
        ...
        Tf_cpu  = gen_btjc_cpu.size(0)
        mv_pred = frame_disp_cpu(gen_btjc_cpu)
        mv_gt   = frame_disp_cpu(fut_gt_cpu)
        print(f"[GEN] Tf={Tf_cpu}, mean|Δpred|={mv_pred:.4f}, mean|Δgt|={mv_gt:.4f}")
        if Tf_cpu < 2:
            print("[WARN] predicted sequence length < 2; animation may look static.")
