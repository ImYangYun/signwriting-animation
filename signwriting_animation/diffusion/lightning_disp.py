...
"""
Lightning module with Displacement Loss for better motion magnitude control.
"""
import os
import torch
...
class LitDiffusion(pl.LightningModule):
    ...
    NEW: Added displacement loss for better motion magnitude control.
    ...
    Args:
        ...
        disp_weight: Weight for displacement loss term (NEW)
    ...
    def __init__(
        ...
        disp_weight: float = 1.0,  # NEW: displacement loss weight
        t_past: int = 40,
        t_future: int = 20,
    ):
        super().__init__()
        ...
        self.disp_weight = disp_weight  # NEW
        self._step_count = 0
        ...
        self.lr = lr
        self.train_logs = {"loss": [], "mse": [], "vel": [], "acc": [], "disp": [], "disp_ratio": []}
    ...
    def training_step(self, batch: dict, _batch_idx: int) -> torch.Tensor:
        ...
        4. Compute loss (MSE + velocity + acceleration + displacement)
        ...
        # NEW: Displacement loss (强制幅度匹配)
        pred_disp = pred_vel.abs().mean()
        gt_disp = gt_vel.abs().mean()
        loss_disp = torch.abs(pred_disp - gt_disp)
        disp_ratio = (pred_disp / (gt_disp + 1e-8)).item()  # for logging only

        # Total loss with displacement term
        loss = (loss_mse 
        ...
        if debug:
            print("\n" + "=" * 70)
            print(f"TRAINING STEP {self._step_count} (with Displacement Loss)")
            print("=" * 70)
            ...
            print(f"  loss_disp: {loss_disp.item():.6f} (weight={self.disp_weight})")
    ...
    def sample(self, past_btjc: torch.Tensor, sign_img: torch.Tensor, future_len: int = 20) -> torch.Tensor:
        ...
        pred_bjct = self.diffusion.p_sample_loop(
            model=wrapped_model,
            shape=target_shape,
            clip_denoised=True,  # Use clip for better results
            model_kwargs={"y": {}},
            progress=False,
        )
    ...
    def on_train_end(self):
        ...
        axes[0, 2].plot(self.train_logs["vel"])
        axes[0, 2].set_title("Velocity Loss")

        axes[1, 0].plot(self.train_logs["acc"])
        axes[1, 0].set_title("Acceleration Loss")

        axes[1, 1].plot(self.train_logs["disp"])
        axes[1, 1].set_title("Displacement Loss")

        axes[1, 2].plot(self.train_logs["disp_ratio"])
        axes[1, 2].set_title("Displacement Ratio (ideal=1.0)")
