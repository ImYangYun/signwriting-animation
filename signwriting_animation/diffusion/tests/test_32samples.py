"""
Past Dropout Training - 32 samples - p30 & p70
完整自包含脚本，直接提交即可

Usage:
    python train_32sample_p30_p70.py
"""
import os
import torch
...
class LitDiffusionPastDropout(pl.LightningModule):
    def __init__(self, num_keypoints=178, num_dims=3, lr=1e-4,
                 ...
                 t_past=40, t_future=20, contrastive_weight=0.5, past_drop_prob=0.3):
        super().__init__()
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self._step_count % 100 == 0:
            ...
            print(f"[Step {self._step_count}]{drop_str} loss={loss.item():.4f}, disp_ratio={disp_ratio:.4f}")
...
# Training Function
# ============================================================
def train_single(dropout_rate, cached_ds, num_joints, num_dims, future_len):
    dropout_str = f"p{int(dropout_rate * 100)}"
    out_dir = f"logs/past_dropout_fixed_{NUM_SAMPLES}sample_{dropout_str}"
    os.makedirs(out_dir, exist_ok=True)

    print("\n" + "=" * 70)
    print(f" TRAINING: {NUM_SAMPLES} samples, dropout={dropout_rate} ({dropout_str})")
    print("=" * 70)

    train_loader = DataLoader(
        cached_ds, batch_size=min(NUM_SAMPLES, 8), 
    ...
    lit_model = LitDiffusionPastDropout(
        ...
        contrastive_weight=0.5, past_drop_prob=dropout_rate,
    )
    ...
    for cache_idx in range(min(5, len(cached_ds))):
        ...
        print(f"idx={meta['idx']}: ratio={ratio:.2f}, PCK={pck:.1f}%")
        results.append({"idx": meta["idx"], "ratio": ratio, "pck": pck, "meta": meta})
    ...
    best_idx = [i for i, r in enumerate(results) if r["idx"] == best["idx"]][0]
    
    sample = cached_ds.samples[best_idx]
    meta = cached_ds.get_metadata(best_idx)
    ...
    print(f"\nBest (PCK={best['pck']:.1f}%) saved to: {out_dir}/test_*.pose")
    ...
    return {"dropout": dropout_str, "avg_pck": avg_pck, "avg_ratio": avg_ratio}
...
def main():
    ...
    print("=" * 70)
    print(f" PAST DROPOUT TRAINING - {NUM_SAMPLES} samples")
    ...
    for dropout_rate in DROPOUT_RATES:
        result = train_single(dropout_rate, cached_ds, num_joints, num_dims, future_len)
    ...
    print(" FINAL SUMMARY")
    print("=" * 70)
    print(f"{'Config':<10} {'AVG PCK':<15} {'AVG Ratio':<15}")
    print("-" * 40)
    for r in all_results:
        print(f"{r['dropout']:<10} {r['avg_pck']:<15.1f} {r['avg_ratio']:<15.2f}")
    print("=" * 70)
