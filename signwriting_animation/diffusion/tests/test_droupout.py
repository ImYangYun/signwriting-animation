"""
Past Dropout Training - FIXED VERSION with CachedDataset

Fixes:
- Uses CachedDataset to ensure train/test consistency  
- Fixed sanitize_btjc for MaskedTensor
"""
import os
import argparse
import torch
...
def train_overfit(args):
    ...
    out_dir = f"logs/past_dropout_fixed_{args.num_samples}sample_p{int(args.past_drop_prob*100)}"

    os.makedirs(out_dir, exist_ok=True)

    print("=" * 70)
    print(f" PAST DROPOUT TRAINING - FIXED (p={args.past_drop_prob})")
    print("=" * 70)
    ...
    train_loader = DataLoader(
        cached_ds, batch_size=min(args.num_samples, 8), 
    ...
    lit_model = LitDiffusionPastDropout(
        ...
        contrastive_weight=0.5, past_drop_prob=args.past_drop_prob,
    )
    ...
    # ============================================================
    print("\n" + "=" * 70)
    print("TESTING ON CACHED SAMPLES")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    if avg_pck > 50:
        print("✅ Past Dropout OVERFIT WORKS!")
    elif avg_pck > 30:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--past_drop_prob", type=float, default=0.7)
